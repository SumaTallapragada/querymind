"""Exports business knowledge models to JSON, YAML, or a plain dict.

This is the boundary any future consumer (Prompt Builder, Admin UI,
documentation generator, a future Schema Linker integration) is expected
to read through — a dict/JSON/YAML string produced here, never a live
Pydantic object. Mirrors `querymind.metadata.serializer.MetadataSerializer`
exactly.
"""
...
class BusinessKnowledgeSerializer:
    """Stateless exporter for any business knowledge Pydantic model (e.g. `BusinessConcept`).
    ...
    def to_yaml(model: BaseModel) -> str:
        ...
        return yaml.safe_dump(
            BusinessKnowledgeSerializer.to_dict(model), sort_keys=False, allow_unicode=True
        )
