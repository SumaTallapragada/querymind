"""Exports orchestrator models to JSON, YAML, or a plain dict.

Mirrors `querymind.result_formatter.serializer.ResultFormatterSerializer` /
`querymind.sql_execution.serializer.SQLExecutionSerializer`-style
...
class QueryMindSerializer:
    """Stateless exporter for any orchestrator Pydantic model (e.g. `QueryMindResponse`).
    ...
    def to_yaml(model: BaseModel) -> str:
        ...
        return yaml.safe_dump(
            QueryMindSerializer.to_dict(model), sort_keys=False, allow_unicode=True
        )
