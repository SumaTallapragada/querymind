"""Exports prompt compiler models to JSON, YAML, or a plain dict.

This is the boundary any future consumer (the LLM Adapter, an admin UI,
a debugging tool) is expected to read through — a dict/JSON/YAML string
produced here, never a live Pydantic object. Mirrors
`querymind.query_library.serializer.QueryLibrarySerializer` exactly.
"""
...
class PromptCompilerSerializer:
    """Stateless exporter for any prompt compiler Pydantic model (e.g. `CompiledPrompt`).
    ...
    def to_yaml(model: BaseModel) -> str:
        ...
        return yaml.safe_dump(
            PromptCompilerSerializer.to_dict(model), sort_keys=False, allow_unicode=True
        )
