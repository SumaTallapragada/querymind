"""Exports result_formatter models to JSON, YAML, or a plain dict.

Mirrors `querymind.sql_execution.serializer.SQLExecutionSerializer` /
`querymind.sql_repair.serializer.SQLRepairSerializer`-style exporters
used throughout this project exactly.
"""
...
class ResultFormatterSerializer:
    """Stateless exporter for any result_formatter Pydantic model (e.g. `BusinessAnswer`).
    ...
    def to_yaml(model: BaseModel) -> str:
        ...
        return yaml.safe_dump(
            ResultFormatterSerializer.to_dict(model), sort_keys=False, allow_unicode=True
        )
