"""Exports observability models to JSON, YAML, or a plain dict.

Mirrors `querymind.orchestrator.serializer.QueryMindSerializer` /
`querymind.result_formatter.serializer.ResultFormatterSerializer`-style
...
class ObservabilitySerializer:
    """Stateless exporter for any observability Pydantic model (e.g. `MetricsSnapshot`).
    ...
    def to_yaml(model: BaseModel) -> str:
        ...
        return yaml.safe_dump(
            ObservabilitySerializer.to_dict(model), sort_keys=False, allow_unicode=True
        )
